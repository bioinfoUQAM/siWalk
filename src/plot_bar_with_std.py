'''
Bar chart visualization of ML model performance with standard deviation.

Reads a cross-validation report TSV and plots one bar chart per metric
(MCC, accuracy, precision, etc.), sorted by performance descending, with
error bars representing standard deviation across folds. Saves to PNG.
...
def run(infile):
  """Plot per-metric bar charts with std error bars from a CV report TSV.
