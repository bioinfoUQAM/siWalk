'''
Classify siRNA precursor candidates using a pre-trained pickled model.

Loads a feature-annotated TSV, applies the same preprocessing and top-100
chi2 feature selection used during training, and predicts class labels and
probabilities with a pre-trained pickled classifier (GBA100 or RFAs100).
Writes predictions to a new TSV alongside the original data.
...
# columns to drop for localization prediction (structural features only)
...
def classify_a_file(infile2, pickle_file, datafile, cols_to_drop, cols2drop_for_locapredi=cols2drop_for_locapredi):
  """Predict siRNA precursor labels for all instances in *infile2* using the pickled model.

  Writes a prediction TSV with Probability_FALSE, Probability_TRUE, and Predicted_Class columns.
  The k used here must match the k used when training the model.
  """
  k = 100
...
if __name__ == "__main__":
  if len(sys.argv) < 2:
    ...
    print("example usage:")
    print("annotation_file=../Unittest_pickle_classifyAfile/GSM1087987_C0FCSb.contig_features.addref.rm_nonfeat.consistent.tsv")
    print("ref=../model/Arabidopsis_strcture_feature_importance_n_correlation.tsv")
    print("pickle_file=../model/RFAs100.pkl")
    print("python siWalk_classify_precursors.py $annotation_file $ref $pickle_file\n")
    print("Existing...")
    sys.exit(0)

  file = sys.argv[1]
  datafile = sys.argv[2] if len(sys.argv) > 2 else '../model/Arabidopsis_feature_importance_n_correlation.tsv'
  pickle_file = sys.argv[3] if len(sys.argv) > 3 else '../model/GBA100.pkl'

  print("Predicting siRNA precursor from library annotation file: \n", file)
  classify_a_file(file, pickle_file, datafile, cols_to_drop)
  pass
