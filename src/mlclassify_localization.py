'''
siRNA localization classifier (Module B helper).

Classifies each candidate siRNA window within a precursor using the RFAs100
model (RandomForest+AdaBoost, top-100 structural features). Returns a list of
start positions predicted as siRNA-generating loci.
...
def concatenate(df1, df2):
  """Concatenate df1 and df2, filling missing columns from df2 with zeros."""
...
def get_data(datafile, k=100):
  """Load feature importance and correlation from *datafile* and return top-k names."""
...
def classify_a_file(infile2, pickle_file=pickle_file, datafile=datafile, cols_to_drop=cols_to_drop):
  """
  Classify siRNA candidate windows and return predicted start positions.
