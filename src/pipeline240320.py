...
# time (DD-HH:MM), # 0-00:05 = 5 mins
#
salloc --job-name=pipeline240320_ath_lib \
 --time=03:00:00 \
 --nodes=1 \
 --ntasks-per-node=1 \
 --cpus-per-task=2 \
...
## WHEAT
INPUT_SAM_PATH=/home/cjwu/scratch/pipeline240320/input_wheatlib1/
genome_file=/home/cjwu/scratch/siWalkPrototype/dbs/wheat_genome/Genome/WHEAT_IWGSC_21chrs.fa
ref=/home/cjwu/scratch/pipeline240320/dbs/Tae_phasing_contig_from_literature.txt
sampling=-1



time spark-submit --master local[*] --executor-memory ${SLURM_MEM_PER_NODE}M \
             ../src/pipeline240320.py \
             --jobid $SLURM_JOBID \
             --input_path $INPUT_SAM_PATH --genome_file $genome_file \
             --ref_file $ref \
             --sampling $sampling


git commit -m "debug" && git push origin master

git checkout *.sh ../src/pipeline240320.py && \
git pull origin master && dos2unix *.sh
sbatch raw_bowtie_alignment_2.sh #submit.sh
squeue -u cjwu


git pull origin master && sbatch raw_bowtie_alignment_2.sh
...
#
jobid=32085568
libid=GSM1087987_C0FCSb
rm GSM*
##cp ../output/$jobid/$libid/$libid.*summary* .
cp ../output/$jobid/$libid/$libid.*contig_features* .
rm $libid.*contig_features.addref.tsv 
cp ../output/$jobid/$libid/$libid.*3__5860000_5865250* .
#cp ../output/$jobid/$libid/$libid.*1__18545000_18550250* .
#cp ../output/$jobid/$libid/$libid.*2__16535000_16540250* .
cp ../output/$jobid/$libid/$libid.*5__20130000_20135250* .
cp ../output/$jobid/$libid/$libid.miranda_scores.tsv .
head -n 2 $libid*

wc -l $libid.*contig_features.tsv
head -1 $libid.*contig_features.tsv | awk -F'\t' '{print NF}'
wc -l $libid.*contig_features.addref.rm_nonfeat.tsv
head -1 $libid.*contig_features.addref.rm_nonfeat.tsv | awk -F'\t' '{print NF}'
ls $libid.*
...
jobid=32085497
#rm GSM*
cp ../output/$jobid*/*/*contig_features* .
rm *contig_features.addref.tsv 
cp ../output/$jobid*/*/*3__5860000_5865250* .
cp ../output/$jobid*/*/*5__20130000_20135250* .
cp ../output/$jobid*/*/*.miranda_scores.tsv .

wc -l *contig_features.tsv
head -1 *contig_features.tsv | awk -F'\t' '{print NF}'
wc -l *contig_features.addref.rm_nonfeat.tsv
head -1 *contig_features.addref.rm_nonfeat.tsv | awk -F'\t' '{print NF}'
...
######################################################
DicerCall=24 takes a lot more time thatn DicerCall=21
######################################################
